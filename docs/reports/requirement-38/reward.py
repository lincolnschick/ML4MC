...
# class RewardForDamagingEntity(RewardHandler):
#     """
#     Insert Text
#     """

#     def to_string(self) -> str:
#         return "reward_for_damaging_a_specific_mob"
    
#     def xml_template(self) -> str:
#         return str(
#             """<RewardForDamagingEntity>
#                     {% for mob in mob_list %}
#                     <Mob type="{{ mob.mob }}" reward={{ mob.reward }}>
#                     {% endfor %}
#                 </RewardForDamagingEntity>
#             """
#         )
    
#     def __init__(self, mob_rewards: List[Dict[str, float]]):
#         self.mob_list = mob_rewards
#         self.reward_dict = {
#             a['mob']: dict(reward=a['reward']) for a in self.items
#         }

#         # Assert all items have the appropriate fields for the XML template.
#         for mob in self.mob_list:
#             assert set(mob.keys()) == {"reward", "mob"}
...
class RewardForXPGain(RewardHandler):
    """
    Implements a custom reward for the AI agent when the agent gains XP within the world of Minecraft
    """

    def to_string(self) -> str:
        return "reward_for_gaining_xp"
    ...
    def xml_template(self) -> str:
        ...
        #                 Start Choice [1..*]
        #                     <Mob> MobWithReward </Mob> [1]
        #                 End Choice
        #         </RewardForDamagingEntity>"""
        # )

    def __init__(self, reward_per_xp: float, reward_type: str = 'FIXED'):
        super().__init__()
        self.xpReward = reward_per_xp
        self.currXP = 0
        self.rewardType = reward_type

    def from_universal(self, obs):
        if 'life_stats' in obs and 'xp' in obs['life_stats']: #Pulls up the NPCs life stats
                if self.currXP <= obs['life_stats']['xp']: #finds if the XP total has changed.
                    if self.rewardType == 'FIXED': #Implements a fixed XP gain
                        self.currXP = obs['life_stats']['xp']
                        return self.xpReward
                    elif self.rewardType == 'VARIABLE': #Implements a variable XP gain (gain per XP point)
                        xpDiff = obs['life_stats']['xp'] - self.currXP
                        self.currXP = obs['life_stats']['xp']
                        return xpDiff * self.xpReward
        return 0
                    
class RewardForTakingDMG(RewardHandler):
    """
    Implements a custom reward for the AI agent when the Agent takes damage. 
    """

    def to_string(self) -> str:
        return "reward_for_taking_damage"
    
    def xml_template(self) -> str:
        return ""
        # """<RewardForTakingDMG>
        #         <Reward reward="{{ dmgReward }}" description="{{ rewardType }}" />
        #     </RewardForTakingDMG>"""


    def __init__(self, reward_per_dmg: float, reward_type: str = "FIXED"):
        super().__init__()
        self.dmgReward = reward_per_dmg
        self.currLife = 20.0
        self.rewardType = reward_type
    
    def from_universal(self, obs):
        # raise RuntimeError("From_univeral execution reached!")
        if 'life_stats' in obs and 'life' in obs['life_stats']: #pulls up the agent's life stats
            if self.currLife != obs['life_stats']['life']: #compares the current HP to the previous step to determine if the value has changed
                lifeDiff = obs['life_Stasts']['life'] - self.currLife
                self.currLife = obs['life_stats']['life']
                if self.rewardType == 'FIXED' and lifeDiff < 0: #Gives a fixed reward for taking damge
                    return self.dmgReward
                elif self.rewardType == 'VARIABLE' and lifeDiff < 0: #gives a variable reward for taking dame (varies with the amount of damage taken)
                    return self.dmgReward * lifeDiff
        return 0
