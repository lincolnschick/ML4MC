...
TRAIN_MODEL_NAME = 'bc.pth'  # name to use when saving the trained agent.
TEST_MODEL_NAME = 'bc.pth'  # name to use when loading the trained agent.
...
def train():
    ...
    for dataset_obs, dataset_actions, _, _, _ in tqdm(data.batch_iter(num_epochs=EPOCHS, batch_size=32, seq_len=1)):
        ...
        logits = network(th.from_numpy(obs).float())

        # Minimize cross-entropy with target labels.
        # We could also compute the probability of demonstration actions and
        # maximize them.
        loss = loss_function(logits, th.from_numpy(actions).long())
...
def get_action_sequence():
    ...
    # Jump to place crafting table
    action_sequence += ['jump']
    action_sequence += [''] * 5
    action_sequence += ['place:crafting_table']
    action_sequence += [''] * 10

    # Craft furnace and stone pickaxe and equip stone pickaxe
    action_sequence += ['camera:[-1,0]']
